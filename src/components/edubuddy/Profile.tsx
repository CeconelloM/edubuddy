...
export function Profile({
  ...
  const pct = Math.round((goalDone / goalTotal) * 100);
  const r = 46;
  const c = 2 * Math.PI * r;
      ...
      <div className="flex-1 space-y-6 px-6 pb-8">
        <div className="flex items-center gap-4 rounded-3xl bg-surface p-5 ring-1 ring-border">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-muted-foreground">Member since October 2026</p>
          </div>
        </div>
          ...
          <div className="space-y-1 rounded-2xl bg-surface p-2 ring-1 ring-border">
            {LEVELS.map((l, i) => {
              ...
              return (
                ...
                >
                  <span className={`text-sm ${active ? "font-semibold text-brand-green" : "text-foreground"}`}>{l}</span>
                  <span
        ...
        <section>
          ...
          <div className="flex items-center gap-5 rounded-3xl bg-surface p-6 ring-1 ring-border">
            ...
            <div className="min-w-0">
              <p className="inline-flex items-center gap-1.5 rounded-full bg-brand-orange-soft px-2.5 py-1 text-xs font-semibold text-brand-orange">
                <Flame className="size-3.5" /> 4-day streak
              </p>
              <p className="mt-2 text-sm font-semibold text-foreground">Daily Practice</p>
              <p className="text-xs text-muted-foreground text-pretty">
                {goalDone} of {goalTotal} messages sent — keep going, you're almost there!
              </p>
            </div>
          </div>
        </section>
